...
const db_1 = require("./db"); // Assuming you've put the MySQL class in a file named db.ts
...
const SSH_CONFIG = {
    ...
    privateKey: fs_1.default.readFileSync(process.env.SSH_PRIVATE_KEY_PATH),
...
async function main() {
    ...
    try {
        ...
        const tunnelStream = await new Promise((resolve, reject) => {
        ...
        mysql = new db_1.MySQL({ ...DB_CONFIG, stream: tunnelStream });
        if (debugMode)
            console.log('Successfully connected to the database over SSH.\n');
...
async function generatePrompt(mysql, mauticError, constraintName) {
    const query = `
    SELECT 
  ...
  `;
    ...
    async function getSampleData(tableName, limit = 5) {
        try {
            ...
            console.error(`Error fetching sample data from ${tableName}: ${error.message}`);
            return [];
        }
    }
